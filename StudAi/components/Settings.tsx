...
const Settings: React.FC<SettingsProps> = ({ user, onUpdate }) => {
  ...
  const handleToggleContrast = async () => {
    const newUser = await AppController.toggleHighContrast(user.id, prefs.highContrast);
    if (newUser) {
  ...
  const handleFontSizeChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const newUser = await AppController.updatePreferences(user.id, { fontSize: newVal });
    if (newUser) {
        setPrefs(newUser.preferences);
        onUpdate(newUser);
    }
  ...
  const handleVoiceSpeedChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const newVal = parseFloat(e.target.value);
    const newUser = await AppController.updatePreferences(user.id, { voiceSpeed: newVal });
    if (newUser) {
        setPrefs(newUser.preferences);
        onUpdate(newUser);
    }
  ...
  const handleConsentChange = async (key: keyof User['consents']) => {
    const newConsents = { ...consents, [key]: !consents[key] };
    const newUser = await AppController.updateConsents(user.id, newConsents);
    if (newUser) {
  ...
  const handleSaveProfile = async () => {
    const newUser = await AppController.updateProfile(user.id, {
        name: profileName,
        email: profileEmail
    });
    if (newUser) {
